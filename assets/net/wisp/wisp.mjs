...
class WispStream extends EventTarget {
  ...
  send(data) {
    const canSend = this.buffer_size == null || this.buffer_size > 0 || !this.open || this.stream_type === 0x02;
    if (canSend) {
      let packet = create_packet(0x02, this.stream_id, data);
      this.ws.send(packet);
      if (this.buffer_size > 0) this.buffer_size--;
    }
    else {
...
export class WispConnection extends EventTarget {
  constructor(wisp_url) {
    ...
    if (!this.wisp_url.split("?")[0].endsWith("/")) {
